...
The included `.env` targets the Nile Testnet. For local overrides, use `.env.local` and keep API keys or other secrets out of committed files.
...
The example passes the amount to `tronWeb.transactionBuilder.sendTrx`. TronWeb supports **36 transaction types**; replace the transaction in Step 3 to send other types.
